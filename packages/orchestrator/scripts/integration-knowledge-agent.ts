...
async function main() {
  ...
  try {
    ...
    const identity = await run("curator", [call("knowledge.inspect_duplicates", { label: "Motor", type: "artifact" }), call("knowledge.propose_merge", { fromId: sameA.id, intoId: sameB.id, rationale: "review" }), done()]); assert((identity.result.steps[0].result.data as { autoMerged: boolean }).autoMerged === false && (await canonical.getNode(sameA.id))?.status === "accepted", "duplicate inspection and semantic similarity never auto-merge ambiguous same-label identities"); const mergeProposal = (await canonical.listProposals({ status: "pending" })).find((p) => identity.result.proposalIds.includes(p.id)); assert(mergeProposal?.kind === "merge", "Curator proposes merge without executing it");
