...
export const DEFAULT_KNOWLEDGE_EMBEDDING_MODEL = "qwen3-embedding:4b";
const KNOWN_NATIVE_DIMENSIONS: Readonly<Record<string, number>> = { "qwen3-embedding:0.6b": 1024, "qwen3-embedding:4b": 2560 };
...
export function resolveKnowledgeEmbeddingConfig(env: NodeJS.ProcessEnv = process.env): KnowledgeEmbeddingConfig {
  ...
  const model = env.KNOWLEDGE_EMBEDDING_MODEL?.trim() || DEFAULT_KNOWLEDGE_EMBEDDING_MODEL;
  ...
  const nativeDimension = KNOWN_NATIVE_DIMENSIONS[model];
  if (nativeDimension !== undefined && nativeDimension < dimension) throw new Error(`Ollama embedding model ${model} has native dimension ${nativeDimension}, below requested KNOWLEDGE_VECTOR_DIMENSION ${dimension}`);
