...
Knowledge path (M12+): tools in the loop; optional neighborhood/project-status inject; ingest/FP/continuous capture produce **proposals** only; accept remains explicit. Continuous capture uses conversation-optimised extract (post-M18 iteration `7d474bb`), not only generic batch ingest. Voice owns streaming microphone/VAD/STT and cancellable TTS/playback mechanics, while authorized final speech still enters the same `handle()` and durable experience path. The command-backed local STT fallback closes bounded VAD segments and honestly exposes no partial-transcript capability. TTS remains optional and default off. Same brain; no second orchestrator in UI or HTTP.
