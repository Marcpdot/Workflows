...
- Orchestrator **runtime/type imports** of extracted layers use `file:../packages/<name>` as `@workflows/<name>` (e.g. `@workflows/eval/cost`, `@workflows/models/local`, `@workflows/memory`) so they resolve under `node_modules`, not under Orchestrator’s `rootDir`.
